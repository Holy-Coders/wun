(ns myapp.server.auth
  "Server-side auth: bcrypt-hashed users, opaque session tokens.
   Backed by next.jdbc against SQLite. Schema lives in
   `resources/migrations/0002_create_users.sql`."
...
(defn init!
  "No-op -- the users / sessions tables are created by the migration
   runner at boot. Defined so server/main.clj can call (auth/init!)
   uniformly regardless of whether init becomes non-trivial later."
  [] :ok)
...
(defn sign-up! [email password]
  ...
  (let [hash (hashers/derive password)
        {:users/keys [id]
         :as         row}
        (sql/insert! (db/ds) :users
                     {:email email :password_hash hash}
                     {:return-keys true})
        token (random-token)]
    (sql/insert! (db/ds) :sessions {:token token :user_id (or id (:id row))})
    {:user-id (or id (:id row)) :token token}))
