...
(deftest apply-intent-runs-morph
  (intents/definent ::add {:params [:map [:n :int]]
...
(deftest apply-intent-no-op-on-validation-failure
  (intents/definent ::strict {:params [:map [:n :int]]
...
(deftest server-only-flag
  (intents/definent ::auth {:server-only? true :params [:map] :morph (fn [s _] s)})
  (intents/definent ::not-auth {:params [:map] :morph (fn [s _] s)})
