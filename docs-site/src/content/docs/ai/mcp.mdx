...
> **TL;DR.** A stdio Model Context Protocol server that exposes
> the `wun` CLI's scaffolders + status to any MCP-aware agent
> (Claude Desktop, Cursor, Cline, Continue). Plus read-only
> resources for `CLAUDE.md`, the architecture docs, and every
> skill.
