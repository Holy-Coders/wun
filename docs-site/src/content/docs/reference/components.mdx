...
> **TL;DR.** This is the foundational vocabulary that ships with
> the framework. Every entry has a native renderer on web, iOS,
> and Android, plus a server-side HTML mapping so the WebFrame
